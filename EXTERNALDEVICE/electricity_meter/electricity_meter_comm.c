...
void ElectricityMeterRecvAndHandleFrameStruct(void)
{
	...
	if(xResult == pdPASS )
	{
		...
		{
			case 0x0400:	//数据透传
				ElectricityMeterTransparentTransmission(server_frame_struct);
			break;
		...
		for(j = 0; j < server_frame_struct->para_num; j ++)
		{
			if(server_frame_struct->para[j].type == 0x3005)
			{
				if(server_frame_struct->para[j].len % 2 == 0)
				{
					...
					if(frame != NULL)
					{
						frame->device_type = ELECTRICITY_METER;
...
//							}
							...
							i = 0;
							msg = msg + 1;
							ElectricityMeterConfig[m].alarm_thre[loop_ch][para_id - 1].min_range = myatoi(tmp);
							...
							i = 0;
							msg = msg + 1;
							ElectricityMeterConfig[m].alarm_thre[loop_ch][para_id - 1].max_range = myatoi(tmp);
					...
					for(k = 0; k < ElectricityMeterConfig[m].ch_num; k ++)
					{
						for(n = 0; n < MAX_ELECTRICITY_METER_ALARM_PARA_NUM; n ++)
						{
							   ...
							   ElectricityMeterConfig[m].alarm_thre[k][n].para_id != 0)
							{
								...
								memset(tmp,0,10);
								sprintf(tmp, "%d",ElectricityMeterConfig[m].alarm_thre[k][n].min_range);
								strcat(buf,tmp);
								strcat(buf,",");
								...
								memset(tmp,0,10);
								sprintf(tmp, "%d",ElectricityMeterConfig[m].alarm_thre[k][n].max_range);
								strcat(buf,tmp);
								strcat(buf,",");
