...
void ElectricityMeterCombineParasThreOverAlarm(u8 i,u8 j,u8 m,double reference_value)
{
	...
	if(alarm_report != NULL)
	{
		...
		if(buf != NULL)
		{
			...
			memset(tmp,0,10);
			sprintf(tmp, "%d",ElectricityMeterConfig[i].alarm_thre[j][m].min_range);
			strcat(buf,tmp);
			strcat(buf,",");
			...
			memset(tmp,0,10);
			sprintf(tmp, "%d",ElectricityMeterConfig[i].alarm_thre[j][m].max_range);
			strcat(buf,tmp);
			strcat(buf,",");
					...
					for(m = 0; m < MAX_ELECTRICITY_METER_ALARM_PARA_NUM; m ++)
					{
						   ...
						   ElectricityMeterConfig[i].alarm_thre[j][m].para_id != 0)
						{
							...
							if(max_value != 0xFFFFFFFFFFFFFFFF && ElectricityMeterConfig[i].alarm_thre[j][m].max_value != 0.0f)
							{
								...
								{
									if((value - ElectricityMeterConfig[i].alarm_thre[j][m].max_value) / 100.0f >= 
									  ((float)ElectricityMeterConfig[i].alarm_thre[j][m].max_range / 100.0f))
									{
										cnt[i][j].cnt[m] --;
					...
					for(m = 0; m < MAX_ELECTRICITY_METER_ALARM_PARA_NUM; m ++)
					{
						   ...
						   ElectricityMeterConfig[i].alarm_thre[j][m].para_id != 0)
						{
							...
							if(min_value != 0xFFFFFFFFFFFFFFFF && ElectricityMeterConfig[i].alarm_thre[j][m].max_value != 0.0f)
							{
								...
								{
									if((ElectricityMeterConfig[i].alarm_thre[j][m].min_value - value) / 100.0f >= 
									  ((float)ElectricityMeterConfig[i].alarm_thre[j][m].max_range / 100.0f))
									{
										cnt[i][j].cnt[m] --;
